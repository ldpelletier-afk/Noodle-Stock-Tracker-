...
const APP_FILE: &str = "portfolio_app.py";
const PORT: u16 = 8501;
...
/// Resolves the `streamlit` binary.
///
/// GUI apps launched from Finder/Dock inherit only launchd's minimal PATH, not
/// the user's shell PATH — so a bare `streamlit` lookup fails even when it's
/// installed. We try, in order:
///   1. The user's interactive login shell PATH (`zsh -ilc`), which covers
///      Homebrew, pyenv, and venvs activated in `.zshrc`/`.zprofile`.
///   2. A list of well-known absolute install locations, so a default
///      python.org or Homebrew install works with no shell config at all.
fn resolve_streamlit_bin() -> Option<String> {
    if let Some(output) = Command::new("/bin/zsh")
        .args(["-ilc", "command -v streamlit"])
        .output()
        .ok()
    {
    ...
    let candidates = [
        "/Library/Frameworks/Python.framework/Versions/3.13/bin/streamlit".to_string(),
        "/opt/homebrew/bin/streamlit".to_string(),
        "/usr/local/bin/streamlit".to_string(),
        format!("{home}/.local/bin/streamlit"),
    ];
...
fn ensure_server_running() -> bool {
    ...
    let log_file = OpenOptions::new()
        .create(true)
        .append(true)
        .open(format!("{log_dir}/portfolio-server.log"))
        .ok();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            .title("Noodle Stock Tracker — Portfolio")
